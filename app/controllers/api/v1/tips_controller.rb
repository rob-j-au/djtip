  ...
  module V1
    class TipsController < Api::V1::BaseController
      ...
      def index
        ...
        render json: TipSerializer.new(@tips, include: %i[event user]).serializable_hash
      end

      # GET /api/v1/tips/:id
      def show
        render json: TipSerializer.new(@tip, include: %i[event user]).serializable_hash
      end
      ...
      def create
        ...
        if @tip.save
          render json: TipSerializer.new(@tip, include: %i[event user]).serializable_hash, status: :created
        else
