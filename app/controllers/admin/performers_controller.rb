...
module Admin
  class PerformersController < Admin::BaseController
    ...
    def index
      ...
      @performers = search_collection(@performers, %i[name genre bio], params[:search]) if params[:search].present?

      # Filter by genre
      @performers = @performers.where(genre: params[:genre]) if params[:genre].present?

      # Filter by event
      @performers = @performers.where(event_id: params[:event_id]) if params[:event_id].present?
