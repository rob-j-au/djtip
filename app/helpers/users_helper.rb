    ...
    content_tag :div, class: "avatar#{" #{css_classes}" unless css_classes.empty?}" do
      if user.image_data? && user.image_url(:thumb)
        ...
        content_tag :div, class: 'avatar placeholder' do
          content_tag :div,
                      class: "bg-gradient-to-br from-primary to-secondary text-primary-content rounded-full #{size}" do
    ...
    content_tag :div, class: 'flex items-center gap-3' do
      ...
      concat(
        content_tag :div do
          ...
          concat content_tag(:div, user.email, class: 'text-sm opacity-50') if show_email
        end
      )
    end
  end
